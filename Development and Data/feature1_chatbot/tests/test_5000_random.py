#!/usr/bin/env python3
# feature1_chatbot/tests/test_5000_random.py
"""
Run N random KB queries (default 5000) against the chatbot and produce summary + failures CSV.

Usage (from project root):
  python feature1_chatbot/tests/test_5000_random.py \
    --kb feature1_chatbot/dengue_kb_seed_15000_extended.jsonl \
    --index-dir feature1_chatbot/index \
    --n 5000 --seed 42 --out feature1_chatbot/tests/failures_5000.csv
"""
...
def main():
    ...
    p.add_argument("--n", type=int, default=5000, help="Number of random queries to test (default 5000)")
    p.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility")
    p.add_argument("--out", default="feature1_chatbot/tests/failures_5000.csv", help="Failures CSV output")
    ...
    if args.n <= total_entries:
        sampled_indices = random.sample(range(total_entries), n)
    else:
        # if requested more than available, allow repeats
        sampled_indices = [random.randrange(total_entries) for _ in range(args.n)]
    ...
    for i, idx in enumerate(sampled_indices, start=1):
        ...
        # progress every 100 (5000 run can be long)
        if i % 100 == 0 or i == len(sampled_indices):
            elapsed = time.time() - start_time
            per_item = elapsed / i
            remaining = len(sampled_indices) - i
            eta = int(remaining * per_item)
            print(f"[{i}/{len(sampled_indices)}] idx={idx} status={status} conf={confidence:.3f} elapsed={int(elapsed)}s ETA≈{eta}s")
