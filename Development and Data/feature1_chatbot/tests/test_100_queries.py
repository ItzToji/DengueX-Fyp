...
from feature1_chatbot.chatbot_engine_final import init_index, get_reply, load_kb

def load_kb_queries(kb_path, limit=100):
    # Load up to 'limit' queries from the KB file
    qs = []
    with open(kb_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Pick first variant or fallback to title
...
def main():
    ...
    print("Init index...")
    # Load FAISS index and metadata for chatbot
    ...
    for kb_id, q, expected in queries:
        ...
        # Determine if model returned a matching response
        if exp and len(exp) > 20:
            ok = exp[:20] in rep  # match using excerpt
        else:
